...
sample_images = {
    c_id: clean_img(getattr(skimage.data, c_id)())
    for c_id in [
        "astronaut",
        "camera",
        "checkerboard",
...
def fn_dict(x):
    return [(getattr(f, "__name__", f), f) for f in x]


image_name = ipw.Dropdown(options=sample_images.keys())
noise_func = ipw.Dropdown(
    options=fn_dict(
        [
            salt_and_pepper,
            additive_gaussian,
...
def update_image(*args):
    show_results(
        ...
        noise_level.value,
        filter_func.value,
        {"size": filter_size.value, "neighborhood": filter_shape.value},
    )
    with mpl_out:
...
display(
    ipw.VBox(
        [
            ipw.VBox(
                [
                    ipw.HBox([ipw.Label(value="Image Name:"), image_name]),
