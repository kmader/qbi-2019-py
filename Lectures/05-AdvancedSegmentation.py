...
fig, m_axs = plt.subplots(
    len(m_cell_imgs), 2, figsize=(4 * 2, 4 * len(m_cell_imgs)), dpi=72
)
THRESH_VAL = 0.6
for i, ((ax1, ax2), c_img) in enumerate(zip(m_axs, m_cell_imgs)):
    ax1.hist(
        c_img.ravel()[c_img.ravel() > THRESH_VAL],
        np.linspace(0, 1, 100),
    ...
    ax2.set_title(
        "Volume Fraction (%2.2f%%)" % (100 * np.mean(c_img.ravel() < THRESH_VAL))
    )
    ax2.axis("off")
...
np.random.seed(2018)
fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(12, 4), dpi=72)
