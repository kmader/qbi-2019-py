...
j_img = ax2.imshow(
    neighbor_conv, cmap="nipy_spectral", vmin=1, vmax=4, interpolation="none"
)
plt.colorbar(j_img)
...
ax3.scatter(
    mk_crop["x"], mk_crop["y"], s=mk_crop["width"], alpha=0.25, label="Ground Truth"
)
ax3.legend()
...
ws_neighbors = convolve(
    width_skeleton.astype(int), np.ones((3, 3)), mode="constant", cval=0
)
...
for j_count in np.unique(ws_neighbors[ws_neighbors > 0]):
    ...
    ax1.plot(x_c, y_c, "s", label=j_name.get(j_count, "unknown"), markersize=8)
...
ax5.set_title(
    "Grow First\n{} {}".format(
        "Edges Found:", [x for x in np.unique(e_labs[n_grow_1 > 0]) if x > 0]
    )
)
...
ax6.set_title(
    "Grow Second\n{} {}".format(
        "Edges Found:", [x for x in np.unique(e_labs[n_grow_2 > 0]) if x > 0]
    )
)
    ...
    if len(v_nodes) == 2:
        edge_dict[c_edge] = {
            ...
            "euclidean_distance": np.sqrt(
                np.square(node_dict[v_nodes[0]]["x"] - node_dict[v_nodes[-1]]["x"])
                + np.square(node_dict[v_nodes[0]]["y"] - node_dict[v_nodes[-1]]["y"])
            ),
        ...
        ax1.plot(
            ...
            [s_node["y"], e_node["y"]],
            "b-",
            linewidth=np.mean(dist_map[edge_id_image == c_edge]),
            alpha=0.5,
        )
...
for _, d_values in edge_dict.items():
    ...
    print("Tortuousity: %2.2f" % (d_values["length"] / d_values["euclidean_distance"]))
    s_node = node_dict[v_nodes[0]]
    e_node = node_dict[v_nodes[-1]]
    ax1.plot(
        [s_node["x"], e_node["x"]],
        ...
        "b-",
        linewidth=5,
        alpha=d_values["length"] / d_values["euclidean_distance"],
    )
...
# Plot the surface.
t_xx, t_yy = np.meshgrid(
    np.arange(bw_roi_dmap.shape[1]), np.arange(bw_roi_dmap.shape[0])
)
surf = ax.plot_surface(
    t_xx, t_yy, -1 * bw_roi_dmap, cmap=plt.cm.viridis_r, linewidth=0.1, antialiased=True
)
...
label_area_dict = {
    i: np.sum(ws_labels == i) for i in np.unique(ws_labels[ws_labels > 0])
}
...
fig, (ax1, ax2, ax3) = plt.subplots(1, 3, figsize=(20, 6), dpi=100)
sns.heatmap(
    clean_label_maxi, annot=True, fmt="1.0f", ax=ax1, cbar=False, cmap="gist_earth"
)
ax1.set_title("Local Maxima")
...
print(
    ...
    area_cutoff,
    "Removed",
    np.sum(np.array(lab_areas) < area_cutoff),
    "components",
)
for i, k in label_area_dict.items():
    if k <= area_cutoff:
...
ax2.imshow(mark_boundaries(label_img=ws_labels, image=rgb_img, color=(0, 1, 0)))
ax2.set_title("Watershed")
...
delete_edges = edge_df.query("interface_length>0.33*(start_perimeter+end_perimeter)")
