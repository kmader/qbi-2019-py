...
anim_code = FuncAnimation(
    fig, update_frame, frames=np.linspace(0, 5, 10), interval=1000, repeat_delay=2000
).to_html5_video()
plt.close("all")
HTML(anim_code)
...
for i in range(orb_det.keypoints.shape[0]):
    ax3.plot(
        orb_det.keypoints[i, 1],
        orb_det.keypoints[i, 0],
        "s",
        label="Keypoint {}".format(i),
    )
    ax4.bar(
        np.arange(10) + i / 10.0,
        orb_det.descriptors[i][:10] + 1e-2,
        width=1 / 10.0,
        alpha=0.5,
        label="Keypoint {}".format(i),
    )
