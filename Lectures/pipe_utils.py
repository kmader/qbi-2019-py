...
def show_pipe(pipe, in_data, show_hist=False):
    ...
    last_data = in_data
    for i, (c_ax, (step_name, step_op)) in enumerate(
        zip(m_axs, [("Input Data", None)] + pipe.steps), 1
    ):
...
def add_filters(
    ...
    filt_func=[
        ...
        lambda x: gaussian(x, sigma=5) - gaussian(x, sigma=2),
    ...
    for c_filt in filt_func:
        out_imgs += [
            np.stack(
                [
                    np.stack([c_filt(x[i, :, :, j]) for i in range(n_img)], 0)
...
def add_xy_coord(in_x, polar=False):
    ...
    _, xx, yy, _ = np.meshgrid(
        np.arange(n_img), np.arange(x_dim), np.arange(y_dim), [1], indexing="ij"
    )
    if polar:
...
polar_step = FunctionTransformer(lambda x: add_xy_coord(x, polar=True), validate=False)


def fit_img_pipe(in_pipe, in_x, in_y):
    in_pipe.fit(in_x, px_flatten_func(in_y)[:, 0])
